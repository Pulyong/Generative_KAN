...
class Discriminator(nn.Module):
    def __init__(self, input,hidden,out):
        ...
        self.linear1 = nn.Linear(self.input,self.hidden)
        self.linear2 = nn.Linear(self.hidden, self.hidden)
        self.linear4 = nn.Linear(self.hidden, self.hidden)
        self.linear3 = nn.Linear(self.hidden,self.out)
...
def build_model(config):
    generator = Generator(config['LATENT_SIZE'],config['HIDDEN_SIZE'],config['IMAGE_SIZE'])
    discriminator = Discriminator(config['IMAGE_SIZE'],config['HIDDEN_SIZE'],config['OUTPUT_SIZE'])
...
def main():
    ...
    for epoch in range(1,config['EPOCHS']+1):
        ...
        if epoch % 10 == 0:
            ...
            pimg.save(f'/home/shu/Desktop/Yongjin/GenAI/Project/KAN/vis/GAN{epoch}.png','png')
